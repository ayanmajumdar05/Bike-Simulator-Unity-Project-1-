...
public class AntiStuckSystem : MonoBehaviour
{
    ...
    private BikeControlRemake BikeController;
    private float orgRadius;

    void Awake()
    {
        _wheelCollider = GetComponent<WheelCollider>();
        BikeController = GetComponentInParent<BikeControlRemake>();
        orgRadius = _wheelCollider.radius;
    }

    void Update()
    {
        if (BikeController.currentSpeed < 15.0f && Mathf.Abs(BikeController.LeanRatio) < 0.2f)
        {
            antiWheelStuck();
        }
        else
    ...
    void antiWheelStuck()
    {
        if (useWheelAntiStuck)
        {
            ...
            for (int i = 0; i <= raysNumber; i++)
            {
                ...
                {
                    if (!hit.transform.IsChildOf(BikeController.transform) && !hit.collider.isTrigger)
                    {
                ...
                Debug.DrawRay(wheelModel.position, rayDirection * orgRadius, Color.blue);
                /*
                ...
                {
                    if (!rightHit.transform.IsChildOf(BikeController.transform) && !rightHit.collider.isTrigger)
                    {
                ...
                */
                ...
                {
                    if (!leftHit.transform.IsChildOf(BikeController.transform) && !leftHit.collider.isTrigger)
                    {
